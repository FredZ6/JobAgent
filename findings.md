...
| The first `GET /health` right after the closeout Docker rebuild returned `Recv failure: Connection reset by peer` | Waited for the recreated API container to settle, confirmed `docker compose ps`, and then re-ran the health check successfully before closing out the delivery package |
