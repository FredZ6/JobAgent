...
export default function SubmissionReviewPage() {
  ...
  useEffect(() => {
    ...
    startTransition(async () => {
      try {
        const data = await fetchSubmissionReview(params.id);
        setReview(data);
        setSubmissionNote(data.application.submissionNote);
        try {
          const eventData = await fetchApplicationEvents(params.id, { limit: 20 });
          setEvents(eventData);
        } catch {
          setEvents([]);
        }
  ...
  useEffect(() => {
    ...
    startTransition(async () => {
      try {
        const eventData = await fetchApplicationEvents(params.id, {
  ...
  }, [params.id, loading, historyActorFilter, historySourceFilter, historyEventFilter, historyQuery, historyFrom, historyTo]);
  ...
  const { application, job, resumeVersion, unresolvedFieldCount, failedFieldCount } = review;
